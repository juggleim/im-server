...
type AddUserTagsActor struct {
	bases.BaseActor
}

func (actor *AddUserTagsActor) OnReceive(ctx context.Context, input proto.Message) {
	if req, ok := input.(*pbobjs.UserTagList); ok {
		err := services.AddUserTags(ctx, req)
		if err != nil {
...
func (actor *AddUserTagsActor) CreateInputObj() proto.Message {
