...
type ClearUserTagsActor struct {
	bases.BaseActor
}

func (actor *ClearUserTagsActor) OnReceive(ctx context.Context, input proto.Message) {
	if req, ok := input.(*pbobjs.UserIds); ok {
		err := services.CleanUserTags(ctx, req.UserIds)
		if err != nil {
...
func (actor *ClearUserTagsActor) CreateInputObj() proto.Message {
