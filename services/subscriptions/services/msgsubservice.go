...
func OnlineOfflineHandle(ctx context.Context, msgs *pbobjs.OnlineOfflineMsg) {
	...
	if ok {
		...
		if err == nil { //success
			logs.WithContext(ctx).Infof("msg sub success, userId:%v, err:%v", msgs.UserId, err)
		} else { //failed
			logs.WithContext(ctx).Errorf("msg sub failed, userId:%v, err:%v", msgs.UserId, err)
		}
	}
}
...
func transMentionInfo(mention *pbobjs.MentionInfo) *MentionInfo {
	if mention != nil {
		...
		if mention.MentionType == pbobjs.MentionType_All {
			mentionType = commonservices.MentionType_All
		} else if mention.MentionType == pbobjs.MentionType_Someone {
			mentionType = commonservices.MentionType_Someone
		} else if mention.MentionType == pbobjs.MentionType_AllAndSomeone {
			mentionType = commonservices.MentionType_AllSomeone
		}
		userIds := []string{}
