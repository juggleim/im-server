...
func (conver *GlobalConverDao) UpsertConversation(item models.GlobalConver) error {
	err := dbcommons.GetDb().Exec("INSERT INTO globalconvers(conver_id,sender_id,target_id,channel_type,updated_time,app_key)VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE sender_id=?, target_id=?, updated_time=?",
		item.ConverId, item.SenderId, item.TargetId, item.ChannelType, item.UpdatedTime, item.AppKey, item.SenderId, item.TargetId, item.UpdatedTime).Error
	return err
}

func (conver *GlobalConverDao) QryConversations(appkey, targetId string, channelType pbobjs.ChannelType, startTime int64, count int32, isPositiveOder bool, excludeUserIds []string) ([]*models.GlobalConver, error) {
