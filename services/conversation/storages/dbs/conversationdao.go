...
func (conver *ConversationDao) Upsert(item models.Conversation) error {
	...
	sqlBuilder.WriteString(" (app_key,user_id,target_id,channel_type,sort_time,sync_time,latest_msg_id,latest_msg,latest_unread_msg_index,latest_read_msg_index,latest_read_msg_id,latest_read_msg_time,is_top,top_updated_time,undisturb_type,unread_tag,is_deleted) ")
	...
	sqlBuilder.WriteString("unread_tag=VALUES(unread_tag),is_deleted=VALUES(is_deleted)")
	return dbcommons.GetDb().Exec(sqlBuilder.String(), params...).Error
}

func (conver *ConversationDao) BatchUpsert(items []models.Conversation) error {
	length := len(items)
	if length <= 0 {
		return nil
	}
	...
	sqlBuilder.WriteString("INSERT INTO ")
	sqlBuilder.WriteString(conver.TableName())
	sqlBuilder.WriteString(" (app_key,user_id,target_id,channel_type,sort_time,sync_time,latest_msg_id,latest_msg,latest_unread_msg_index,latest_read_msg_index,latest_read_msg_id,latest_read_msg_time,is_top,top_updated_time,undisturb_type,unread_tag,is_deleted)VALUES")
	...
	sqlBuilder.WriteString("ON DUPLICATE KEY UPDATE sort_time=VALUES(sort_time),sync_time=VALUES(sync_time),latest_msg_id=VALUES(latest_msg_id),latest_msg=VALUES(latest_msg),latest_unread_msg_index=VALUES(latest_unread_msg_index),")
	sqlBuilder.WriteString("latest_read_msg_index=VALUES(latest_read_msg_index),latest_read_msg_id=VALUES(latest_read_msg_id),latest_read_msg_time=VALUES(latest_read_msg_time),")
	sqlBuilder.WriteString("is_top=VALUES(is_top),top_updated_time=VALUES(top_updated_time),undisturb_type=VALUES(undisturb_type),")
	sqlBuilder.WriteString("unread_tag=VALUES(unread_tag),is_deleted=VALUES(is_deleted)")
	return dbcommons.GetDb().Exec(sqlBuilder.String(), params...).Error
...
func (conver *ConversationDao) UpdateLatestReadMsgIndex(appkey, userId, targetId string, channelType pbobjs.ChannelType, msgIndex int64, readMsgId string, readMsgTime int64) (int64, error) {
	...
	upd["unread_tag"] = 0
	result := dbcommons.GetDb().Model(conver).Where("app_key=? and user_id=? and target_id=? and channel_type=? and (latest_read_msg_index<? or unread_tag=1)", appkey, userId, targetId, channelType, msgIndex, msgIndex).Update(upd)
...
func (conver *ConversationDao) ClearUnread(appkey, userId, targetId string, channelType pbobjs.ChannelType) error {
	return dbcommons.GetDb().Exec("update conversations set latest_read_msg_index=latest_unread_msg_index,latest_read_msg_time=(UNIX_TIMESTAMP(NOW(3))*1000),unread_tag=0 where app_key=? and user_id=? and target_id=? and channel_type=?", appkey, userId, targetId, channelType).Error
}
