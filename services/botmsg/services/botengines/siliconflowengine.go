...
func (engine *SiliconFlowEngine) StreamChat(ctx context.Context, senderId, converKey string, question string, f func(part string, sectionStart, sectionend, isFinish bool)) {
...
func (engine *SiliconFlowEngine) Chat(ctx context.Context, senderId, converKey string, question string) string {
	headers := map[string]string{}
	headers["Authorization"] = fmt.Sprintf("Bearer %s", engine.ApiKey)
	headers["Content-Type"] = "application/json"
	req := &SiliconFlowChatReq{
