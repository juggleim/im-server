...
func AddGroupGlobalMuteMembers(ctx *gin.Context) {
	var req models.UserIds
	if err := ctx.BindJSON(&req); err != nil || len(req.UserIds) <= 0 {
		tools.ErrorHttpResp(ctx, errs.IMErrorCode_API_REQ_BODY_ILLEGAL)
		return
	}
	groups := bases.GroupTargets("grp_global_mute", req.UserIds)
	for _, ids := range groups {
		uIds := ids
		go func() {
			bases.AsyncRpcCall(services.ToRpcCtx(ctx, ""), "grp_global_mute", uIds[0], &pbobjs.BatchMuteUsersReq{
				UserIds:  uIds,
				IsDelete: false,
			})
		}()
...
func DelGroupGlobalMuteMembers(ctx *gin.Context) {
	var req models.UserIds
	if err := ctx.BindJSON(&req); err != nil || len(req.UserIds) <= 0 {
		tools.ErrorHttpResp(ctx, errs.IMErrorCode_API_REQ_BODY_ILLEGAL)
		return
...
func QryGroupGlobalMuteMembers(ctx *gin.Context) {
	...
	code, resp, err := bases.SyncRpcCall(services.ToRpcCtx(ctx, ""), "qry_grp_global_mute", tools.RandStr(10), &pbobjs.QryBlockUsersReq{
		Limit:  limit,
		Offset: offset,
	}, func() proto.Message {
