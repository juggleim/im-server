...
Docker Compose 还支持覆盖宿主机端口。下列变量只会修改发布到宿主机的端口，
不会改变容器端口，也不会改变渲染到 YAML 中的 Launcher 配置。

| Compose 变量 | 默认宿主机端口 | 容器端口 |
| --- | --- | --- |
...
例如，执行 `API_HOST_PORT=19001 docker compose up -d` 会将服务端 API 发布到
宿主机的 `19001` 端口，而 Launcher 仍监听容器内的 `9001` 端口。

不同部署方式的差异：

| 模式 | 配置来源 | 重要行为 |
| --- | --- | --- |
| 本地 Docker Compose | `docker-compose.yml` 环境变量经 `launcher/scripts/config_template.yaml` 渲染 | 使用 `mysql` 服务名，在容器内的 `9002` 端口启用导航服务，并默认将各容器端口发布到相同的宿主机端口。可通过上述 `*_HOST_PORT` 变量覆盖宿主机端口。 |
