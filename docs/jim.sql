...
CREATE TABLE `chatroominfos` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uniq_chatid` (`app_key` ASC, `chat_id` ASC) VISIBLE);
...
DROP TABLE IF EXISTS `chatroommembers`;
  ...
  `app_key` VARCHAR(20) NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uniq_memebrid` (`app_key` ASC, `chat_id` ASC, `member_id` ASC) VISIBLE,
  INDEX `idx_memberid` (`app_key` ASC, `member_id` ASC) VISIBLE,
  INDEX `idx_time` (`app_key` ASC, `chat_id` ASC, `created_time` ASC) VISIBLE);

DROP TABLE IF EXISTS `chatroomexts`;
  CREATE TABLE `chatroomexts` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `chat_id` VARCHAR(32) NULL,
  `item_key` VARCHAR(50) NULL,
  `item_value` VARCHAR(1000) NULL,
  `item_type` TINYINT NULL DEFAULT 0,
  `item_time` BIGINT NULL DEFAULT 0,
  `app_key` VARCHAR(20) NULL,
  `member_id` VARCHAR(32) NULL,
  `is_delete` TINYINT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uniq_itemkey` (`app_key` ASC, `chat_id` ASC, `item_key` ASC) VISIBLE,
  INDEX `idx_itemkey` (`app_key` ASC, `chat_id` ASC, `item_type` ASC) VISIBLE);

DROP TABLE IF EXISTS `chrmbanusers`;
  CREATE TABLE `chrmbanusers` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `chat_id` VARCHAR(32) NULL,
  `ban_type` TINYINT NULL,
  `member_id` VARCHAR(32) NULL,
  `created_time` DATETIME(3) NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_time` DATETIME(3) NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `app_key` VARCHAR(20) NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uniq_memberid` (`app_key` ASC, `chat_id` ASC, `ban_type` ASC, `member_id` ASC) VISIBLE);
