...
Docker Compose also accepts host-side port overrides. These variables change
only the published host port; they do not change the container port or any
launcher setting rendered into YAML.

| Compose variable | Default host port | Container port |
| --- | --- | --- |
| `MYSQL_HOST_PORT` | `3306` | `3306` |
| `WS_HOST_PORT` | `9003` | `9003` |
| `API_HOST_PORT` | `9001` | `9001` |
| `NAV_HOST_PORT` | `9002` | `9002` |
| `ADMIN_HOST_PORT` | `8090` | `8090` |
| `PPROF_HOST_PORT` | `6060` | `6060` |

For example, `API_HOST_PORT=19001 docker compose up -d` publishes the Server
API at host port `19001` while the launcher continues listening on container
port `9001`.
...
| --- | --- | --- |
| Local Docker Compose | `docker-compose.yml` environment values rendered through `launcher/scripts/config_template.yaml` | Uses the `mysql` service name, enables Navigator on container port `9002`, and publishes the container ports on their matching host ports by default. The `*_HOST_PORT` variables above override the host side. |
